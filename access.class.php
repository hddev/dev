 ...
 * @version 1.0.0
 		...
 		$out ['RCMEnable'] = "Доступ к RCM";
 		$out ['MEPEnable'] = "Доступ к MEP";
		$out ['EditCourses'] = "Редактирование курсов"; 
		$out ['EditLections'] = "Редактирование лекций"; 
		$out ['EditMaterials'] = "Редактирование материалов";
		$out ['EditTests'] = "Редактирование тестов";
		$out ['AddUsers'] = "Добавление пользователей";
		$out ['EditUsers'] = "Редактирование пользователей";
		$out ['DeleteUsers'] = "Удаление пользователей";
		$out ['AddStudents'] = "Добавление студентов";
		$out ['EditStudents'] = "Редактирование студентов";
		$out ['DeleteStudents'] = "Удаление студентов"; 
...
class Access {
	...
	private function CookiesAccessDelete($id) {
		if ($this -> type == "MYSQL") $query="DELETE FROM `{$this->prefix}_access_table` WHERE `id` = ".$id;
		if ($this -> type == "POSTGRESQL") $query="DELETE FROM {$this->prefix}_access_table WHERE id = ".$id;
	...
	private function IsLoginByCookies() {
		if (!isset($_COOKIE['mixid'])) return false;
		if (strlen($_COOKIE['mixid'])==0) return false;
		
		if ($this -> type == "MYSQL") $query="SELECT * FROM `{$this->prefix}_access_table` WHERE `mix_id` = '".$_COOKIE['mixid']."'";
		if ($this -> type == "POSTGRESQL") $query="SELECT * FROM {$this->prefix}_access_table WHERE mix_id = '".$_COOKIE['mixid']."'";
	...
	public function GetAccessRows() {
		if ($this -> type == "MYSQL") $query="SELECT * FROM `{$this->prefix}_access_table` WHERE `user_id` = ".$this->user_id;
		if ($this -> type == "POSTGRESQL") $query="SELECT * FROM {$this->prefix}_access_table WHERE user_id = ".$this->user_id;
	...
	public function IsUniqueSession() {
		if (SHIsGetRight($this->user_id,'UnlimetedSessionCount')) return true;
	...
	public function GetAccessRowsForHTML() {
		...
		while ($Row=$result->GetRow()) {
			if (isset($Row['session_id'])) {
				$i++;
				if ($Row['session_id']==session_id()) $html=$html."<tr><td><b>".$i.". </b></td><td>&nbsp;</td><td>".$Row['datetime_created']." - текущее подключение</td></tr>";
				else $html=$html."<tr><td><b>".$i.". </b></td><td>&nbsp;</td><td>".$Row['datetime_created']."</td></tr>";
			} else
				$this->CookiesAccessDelete($Row['id']);
		}
		$html=$html."</table>";
	...
	public function CheakSession() {
		...
		if (isset($_SESSION['user']['mixconnection'])) {
			if (isset($_COOKIE['mixconnection'])) {
				if ($_SESSION['user']['mixconnection']==$_COOKIE['mixconnection']) {return true;}
			}
		}
		return false;
	}
	...
	public function DeleteExtraConnections() {
		...
		while ($Row=$result->GetRow()) {
			if (isset($Row['session_id'])) {
				if ($Row['session_id']!=session_id()) $this->CookiesAccessDelete($Row['id']);
			} else
				$this->CookiesAccessDelete($Row['id']);
		}
	}
	...
	private function Authorization_Success($user_id) { // событие вызывается в случае успешной авторизации: прописывает необходимую информацию в сессию и COOKIES
		...
		if ($this -> type == "MYSQL") $query="DELETE FROM `{$this->prefix}_access_table` WHERE `session_id` = '".session_id()."' AND `user_id` =".$user_id;
		if ($this -> type == "POSTGRESQL") $query="DELETE FROM {$this->prefix}_access_table WHERE session_id = '".session_id()."' AND user_id =".$user_id;
			
		$this->db->Commit($query);
		
		//setcookie("PHPSESSID");
		if ($this -> type == "MYSQL") $query="INSERT INTO `{$this->prefix}_access_table` (`user_id`,`session_id`,`ip`,`mix_id`,`cookies_duration`,`datetime_created`)
		VALUES ('{$user_id}','{$session_id}','{$ip}','{$mix_id}','".date("Y-m-d H:i:s",$cookies_duration)."','{$datetime_created}')";
		if ($this -> type == "POSTGRESQL") $query="INSERT INTO {$this->prefix}_access_table (id, user_id,session_id,ip,mix_id,cookies_duration,datetime_created)
		...
		$Log=new Log(0,array('ordercolumn'=>1));
	...
	public function CheackConnection($rekey) {
		if (!isset($rekey)||!isset($_SESSION['rekey'])) return false;
		if ($_SESSION['rekey']==$rekey) return true;
		return false;
	} // CheackConnection
	...
	private function IsAccessExist() {
		if ($this -> type == "MYSQL") $query="SELECT * FROM `{$this->prefix}_access_table` WHERE `user_id` = ".$this->user_id." AND `session_id` = '".session_id()."'";
		if ($this -> type == "POSTGRESQL") $query="SELECT * FROM {$this->prefix}_access_table WHERE user_id = ".$this->user_id." AND session_id = '".session_id()."'";
	...
	public function IsLogin() {// Проверяем была ли сделана авторизация
		if (SHGetCurrentUserID()>0) if ($this->IsAccessExist()) return true;
		return false;
	} // IsLogin
	...
	public function LogIn($username="",$password="") { // Авторизует пользователя в системе.
		$user=new User(0,array("load_user_by_login"=>$username));
		if (null==$user) return false;//throw new ModuleAccessException("Токой пользователь не зарегистрирован, либо пароль неверный!",0,"access");
	...
	public function LogOut() { // Разавторизует пользователя.
		$Log=new Log(0,array('order'=>1));
		$Log->AddSimpleAction("logout");
		
		if (SHGetCurrentUserID()>0) {
			if ($this -> type == "MYSQL") $query="DELETE FROM `{$this->prefix}_access_table` WHERE `session_id` = '".session_id()."' AND `user_id` =".SHGetCurrentUserID();
			if ($this -> type == "POSTGRESQL") $query="DELETE FROM {$this->prefix}_access_table WHERE session_id = '".session_id()."' AND user_id =".SHGetCurrentUserID();
	...
	public function IsAdministrator() { // Проверяет текущего пользователя на принадлежность к группе администраторов
		...
		if (!isset($usergroup)) return false;
		if ($usergroup->GetShortName()=='ADMINS') return true;
		return false;
	}
	...
	public function IsOrganizators() { // Проверяет текущего пользователя на принадлежность к группе организаторов
		$usergroup=new UserGroup($this->GetCurrentUser()->GetGroupId());
		if (!isset($usergroup)) return false;
		if ($usergroup->GetShortName()=='ORGANIZERS') return true;
		if ($usergroup->GetShortName()=='ADMINS') return true;
		return false;
	}
	...
	public function IsInstructors() { // Проверяет текущего пользователя на принадлежность к группе преподавателей
		$usergroup=new UserGroup($this->GetCurrentUser()->GetGroupId());
		if (!isset($usergroup)) return false;
		if ($usergroup->GetShortName()=='INSTRUCTORS') return true;
		if ($usergroup->GetShortName()=='ADMINS') return true;
		return false;
	}
	...
	public function SetProcess($alias) {
		if (isset($alias)) $_SESSION['user']['process']=$alias;
	}
	...
	public function UnSetProcess($alias) {
		if ($this->GetProcess()==$alias) $this->SetProcess("");
	}
	
	public function GetProcess() {
		if (!isset($_SESSION['user']['process'])) return "";
		return $_SESSION['user']['process'];
	}
	
	public function IsProcess($alias) {
		if (!isset($alias)) return false;
		if ($this->GetProcess()==$alias) return true;
		return false;
	}
...
function SHIsLockMode() {
	$access=new Access();
	if ($access->IsAdministrator()) return false;
...
function SHGetCurrentUserID() {
	if (isset($_SESSION['user']['id'])) return intval($_SESSION['user']['id']);
	return -1;
}
...
class Cookie {
    ...
    // период повторного выпуска cookie 
    static $warning = '300'; 
    static $glue = ' | ' ;
    
	public function _construct($userid = false) {
        ...
        if($userid) {
			...
			$buffer = $this->_unpackage($_COOKIE[self::$cookiename]);
				...
				throw new AuthException("Heт cookie-файла");
			}
		}
	}
